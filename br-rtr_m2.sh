#!/bin/bash
###############################################################################
# br-rtr_m2.sh — Модуль 2: BR-RTR — мониторинг (SNMP + Zabbix Agent)
# ОС: Альт Linux
###############################################################################
set -e
...
syslocation BR-RTR au-team.irpo
...
grep -q "^Hostname=BR-RTR" /etc/zabbix/zabbix_agentd.conf || echo "Hostname=BR-RTR" >> /etc/zabbix/zabbix_agentd.conf
...
echo ">>> Настройка мониторинга BR-RTR (Модуль 2) завершена."
